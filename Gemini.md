...
- **プロフィール設定**: ユーザー名やアイコン画像を変更できる機能。(実装済み)
- **アイコン選択機能**: アプリ側で用意した複数の画像からアイコンを選択できる機能。
- **マイランキング履歴**: プロフィール画面で、自分の過去のランキング入賞履歴やハイスコアの推移を確認できる機能。

### 5.2. コース管理・検索機能

- **コースのサムネイル画像設定**: 練習コースにサムネイル画像URLを設定できる機能。(実装済み)
- **画像アップロード機能 (ドラッグ&ドロップ)**: コースのサムネイルやユーザーアイコンを、ドラッグ&ドロップで直感的にアップロードできる機能。
- **練習コースの検索・フィルタリング**: タグ、検索窓、フィルターなどを用いて、練習コースを効率的に検索できる機能。(タイトル/説明文検索・難易度フィルタ実装済み)
- **お気に入りコース登録**: 気に入ったコースをお気に入り登録し、マイページから素早くアクセスできる機能。(実装済み)
- **コース作成時の一括インポート**: CSVやテキストデータから問題を一括で登録できる機能（現状は1問ずつの手動入力）。

### 5.3. 苦手分析と練習の最適化 (重要機能)

- **指の連動性・運指分析**: 前後のキー入力における指の動きを分析し、「同指連打」や「極端に遠いキーへの移動」など、物理的に打ちにくい、またはユーザーが苦手とする指の遷移パターンを特定する機能。
- **入力順序逆転 (Transposition) ミスの検出**: 「tamago」を「tamaog」と打ってしまうような、正しい文字を認識しているが指が先行してしまうタイプのミスを検出し、スピード過多への警告など適切なアドバイスを行う機能。
- **パーソナライズされたコース推薦**: 分析結果に基づき、ユーザーの弱点を改善できる適切な練習コースを推薦する機能。
- **スコアランキング**: データベースを利用し、ユーザーのタイピングスコアをオンラインでランキング表示する機能。(コース別ランキング実装済み)
...
- **ミスタイプ時のペナルティカスタマイズ**:
  - **サウンド効果 (SE/BGM)**: タイプ音、ミス音、クリア時のファンファーレなどの効果音と、BGM設定。
  - 上記ペナルティの有無や種類をコース選択時に選択できる機能。
  - **リアルなタイピング挙動モード**: 通常のエディタのように、間違えた文字も入力され、BackSpaceキーで修正するモード。または、間違えた文字だけをBackSpaceで消せば良い（有効なプレフィックスは残る）モードなどを選択可能にする。
...
- **キーボードショートカット**: モーダル操作（Enterで開始、Escで閉じる）やリトライなどをキーボードのみで完結させる機能。
...
- **ダークモード対応**: 目の疲れを軽減するダークモードへの切り替え機能。
- **SNSシェア機能**: 結果画面からX (Twitter) などへスコアや結果画像をシェアする機能。
...
## 6. 現在の開発状況 (2025/12/07)

現在の開発は主に以下の点に注力しています。

- **高度な苦手分析機能**: ユーザーのタイピングミスを深く分析する機能を実装しました。
  - **データ収集の強化**: 全打鍵履歴 (`keyHistory`) を記録・保存するようにシステムを拡張しました。
  - **分析ロジック**: 「指の連動性（同指連打など）」や「順序逆転（早とちり）」といった質的なミスを検出するアルゴリズムを実装しました。
  - **インサイト表示**: 分析結果に基づき、ユーザーが優先的に取り組むべき課題を「弱点インサイト」カードとして提示するUIを追加しました。
  - **リファクタリング**: `KanaRomajiMap` のロジックを Server/Client Components で安全に利用できるよう分離しました。
- **お気に入り機能**: ユーザーが気に入ったコースを保存できる機能を実装しました。トップページでのお気に入りフィルタリング、コースカード上のハートアイコンによる直感的な操作、マイページでのお気に入り一覧表示が含まれます。
  - **リファクタリング**: コードレビューに基づき、お気に入り切り替え時のトランザクション処理導入（Race Condition対策）、APIのページネーション設定、楽観的UI更新のエラーハンドリング強化、アクセシビリティ対応を行いました。
- **コース検索・フィルタリング機能**: トップページに検索バーと難易度フィルターを実装しました。ユーザーはタイトルや説明文での検索、難易度による絞り込みが可能になり、目的のコースを見つけやすくなりました。
- **コースサムネイル機能**: コースにサムネイル画像URLを設定し、一覧表示時に画像を表示できるようにしました。視覚的にわかりやすく魅力的なコース一覧になりました。
- **APIバリデーション強化**: コース作成・更新APIにおいて、サーバーサイドでの入力データ検証（特にテキスト配列内の必須項目チェック）を強化し、堅牢性を向上させました。
- **リファクタリング**: コース検索時の不要なAPIリクエストを防ぐ競合状態（Race Condition）の解消や、エラーハンドリングの改善を行いました。
- **コースランキング機能**: コース詳細画面（モーダル）に、そのコースのスコアランキングTOP10を表示する機能を実装しました。ユーザーは練習開始前に他のユーザーのスコアを確認し、モチベーションを高めることができます。
  - **リファクタリング**: ランキング機能の実装に伴い、APIの入力バリデーション強化とフロントエンドのエラーハンドリング改善を行いました。
- **プロフィール設定機能**: ユーザー名やアイコン画像を変更できる機能を実装しました。
- **成績履歴画面の刷新**: 期間絞り込み（週・月・年・全期間）、テーブルのページネーション、分析レポートのUI統合を実装し、ユーザビリティを向上させました。
- **共通ヘッダー実装**: 全ページ共通のヘッダーコンポーネントを導入し、ナビゲーションの一貫性と保守性を向上させました。
- **苦手分析機能**: 詳細なミスデータに基づき、苦手なキー、誤打キー、苦手な運指パターン、ミスの種類（隣接キー誤打、逆手誤打など）を分析・可視化する機能を実装しました。これにより、ユーザーは自分の弱点を具体的に把握し、効率的な練習に繋げることができます。
- **不具合修正**:
  - **タイピングロジック**: 誤入力時に正解のプレフィックスまで消えてしまう問題を修正しました。
  - **UI表示**: 次に入力すべき文字のハイライトが正しく表示されない問題を、レンダリングロジックと状態管理の改善により修正しました。
- **運指ガイド機能**: ユーザーが次に打つべきキーに対応する指を視覚的に表示する運指ガイド機能を実装しました。
- **タイピング画面の改善**: タイピング画面にコースタイトルを表示し、レイアウトを調整して視認性を向上させました。
...
## 7. データ構造の変更点 (2025/12/07)

- **DB 変更**:
  - `TypingResult` テーブルに `keyHistory` カラム (JSON文字列) を追加しました。全打鍵ログを保存します。
  - `Favorite` テーブルを追加し、`User` と `Course` の関連を定義しました。
- **API**:
  - `POST /api/typing-results`: `keyHistory` を含むペイロードに対応。
  - `POST /api/courses/[id]/favorite`: お気に入り登録・解除API。
  - `GET /api/user/favorites`: お気に入り一覧取得API。

## 8. 議論・今後の改善方針 (分析の質的向上に向けて)

現在の「回数ベース」の分析には、練習したコースの偏り（特定の文字が多いなど）によるバイアスが含まれるという課題があります。より本質的で公平なスキル診断を実現するため、以下の機能の実装が検討されています。

1.  **診断モード (Calibration Course) の導入**:
    -   全ての指や運指パターンを網羅的に含む「診断専用コース」を用意し、そのプレイデータのみを用いて純粋なスキルを測定する。
2.  **標準化スコア (Normalized Score) の導入**:
    -   各コースの文字出現頻度データを持ち、ミス回数を出現頻度で割ることで「正規化されたミス率」を算出する。これにより、どのコースをプレイしても公平な苦手判定が可能になる。
3.  **相対評価指標の導入**:
    -   ユーザー全体の平均ミス率と比較し、「平均より何倍ミスしやすいか」という相対的な指標で苦手を特定する。

現在はまず「インサイト（質的分析）」と「詳細データ（量的分析）」を提示する機能を実装し、次のフェーズでこれらの抜本的な改善に取り組む方針です。
