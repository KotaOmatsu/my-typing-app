...
- **運指ガイド機能**:
  - タイピング画面に簡易的な指のイラストを表示し、入力すべきキーに応じて適切な指をハイライトする機能。(プレイ前に ON/OFF 可能)
- **ミスタイプ時のペナルティカスタマイズ**:
...
## 7. データ構造の変更点 (2025/12/02)

- **DB 移行**: 静的データ (`src/data/courses.ts`) を廃止し、SQLite データベース (`Course`, `TypingText` テーブル) に移行しました。
- **API**:
